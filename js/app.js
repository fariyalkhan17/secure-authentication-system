 ...
 * Supports both Live Express Backend and GitHub Pages Static Demo Mode
 */
...
function checkAuthState() {
  fetch('/api/auth/me')
    .then(res => {
      if (!res.ok) throw new Error('API Unavailable (Static Mode)');
      return res.json();
    })
    .then(data => {
    ...
    .catch(() => {
      ...
      if (stored) {
        try {
          setLoggedInState(JSON.parse(stored));
          return;
        } catch(e) {}
      }
      setLoggedOutState();
...
function setupAuthForms() {
  ...
  formLogin.addEventListener('submit', (e) => {
    ...
    fetch('/api/auth/login', {
      ...
      .catch(() => {
        // Static Demo Mode Fallback for GitHub Pages
        const isDemoAdmin = email.toLowerCase().includes('admin');
        const demoUser = {
          id: isDemoAdmin ? 2 : 1,
          username: isDemoAdmin ? 'SystemAdmin' : (email.split('@')[0] || 'StandardUser'),
          email: email,
          role: isDemoAdmin ? 'admin' : 'user',
        };
        showToast('Success (Demo Mode)', 'Logged in successfully in Static Interactive Demo Mode.', 'success');
    ...
    fetch('/api/auth/register', {
      ...
      .then(({ status, data }) => {
        if (status === 201) {
          ...
          showToast('Validation Error', data.errors[0].message, 'error');
        } else if (status === 429) {
          showToast('Rate Limited', data.message, 'error');
        } else {
      ...
      .catch(() => {
        ...
        showToast('Account Created (Demo Mode)', 'Registration successful! Logged in automatically.', 'success');
        setLoggedInState(demoUser);
        switchTab('dashboard-tab');
      });
  });
...
function loadUserDashboard() {
  ...
  fetch('/api/user/dashboard')
    .then(res => {
      if (!res.ok) throw new Error(res.status === 401 ? '401' : '404');
      return res.json();
    })
    .then(data => {
      renderDashboardUI(container, data.sessionInfo, data.recentActivity);
    })
    .catch(err => {
      if (err.message === '401') {
        container.innerHTML = `
      ...
      const mockSession = {
        ...
        cookieExpires: new Date(Date.now() + 86400000).toISOString(),
      };

      const mockActivity = [
        { action: 'LOGIN_SUCCESS', status: 'SUCCESS', ip_address: '127.0.0.1', timestamp: new Date().toISOString() },
        { action: 'SESSION_REGENERATED', status: 'SUCCESS', ip_address: '127.0.0.1', timestamp: new Date(Date.now() - 300000).toISOString() },
      ];

      renderDashboardUI(container, mockSession, mockActivity);
    });
}

function renderDashboardUI(container, sessionInfo, recentActivity) {
...
function loadAdminPanel() {
  ...
  fetch('/api/admin/users')
    .then(res => {
      if (!res.ok) throw new Error(res.status === 401 ? '401' : res.status === 403 ? '403' : '404');
      return res.json();
    })
    .then(data => {
    ...
    .catch(err => {
      ...
      if (err.message === '403' || (currentUser && currentUser.role !== 'admin')) {
        container.innerHTML = `
          <div class="card glass-card placeholder-card">
      ...
      // Static Demo Mode Fallback for Admin
      const mockUsers = [
        { id: 1, username: 'StandardUser', email: 'user@example.com', role: 'user', created_at: new Date().toISOString() },
        { id: 2, username: 'SystemAdmin', email: 'admin@example.com', role: 'admin', created_at: new Date().toISOString() }
      ];

      const mockLogs = [
        { id: 101, user_id: 2, username: 'SystemAdmin', action: 'ADMIN_VIEWED_USERS_DIRECTORY', status: 'SUCCESS', ip_address: '127.0.0.1', timestamp: new Date().toISOString() },
        { id: 100, user_id: 1, username: 'StandardUser', action: 'UNAUTHORIZED_ACCESS_ATTEMPT [/api/admin/users]', status: 'BLOCKED', ip_address: '127.0.0.1', timestamp: new Date(Date.now() - 600000).toISOString() },
      ];

      renderAdminPanelUI(container, mockUsers, mockLogs);
    });
}
...
function toggleUserRole(userId, newRole) {
  fetch('/api/admin/change-role', {
    ...
    .catch(() => {
      showToast('Role Updated (Demo)', `User #${userId} role changed to ${newRole}`, 'success');
    });
}
